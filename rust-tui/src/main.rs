...
fn ui(f: &mut Frame, app: &App) {
    ...
    // Status Pane
    let status_color = if app.status == "connected" {
        Color::Green
    } else {
        Color::Yellow
    };
    ...
    for (role, text, partial) in &app.transcripts {
        ...
        let name_style = if role == "assistant" {
            Style::default()
                .fg(Color::Black)
                .bg(Color::Rgb(4, 181, 117))
                .add_modifier(Modifier::BOLD)
        } else {
            Style::default()
                .fg(Color::Black)
                .bg(Color::Rgb(255, 118, 184))
                .add_modifier(Modifier::BOLD)
        };
...
fn wrap_text(text: &str, first_line_max: usize, cont_max: usize) -> Vec<String> {
    ...
    for word in text.split_whitespace() {
        let line_max = if result.is_empty() {
            max_width
        } else {
            cont_max
        };
        if current_line.is_empty() {
            if word.len() > line_max && line_max > 0 {
                ...
                while !remaining.is_empty() {
                    let lm = if result.is_empty() && current_line.is_empty() {
                        max_width
                    } else {
                        cont_max
                    };
...
async fn run_speaker_playback(client: Arc<Client>, ui_tx: mpsc::Sender<UiMsg>) -> Result<()> {
    let host = cpal::default_host();
    let device = host
